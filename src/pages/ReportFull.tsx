    ...
    <Layout profile={profile} onSignOut={signOut} title="Technical Report — Full Document">
      <div style={{ display: 'flex', height: 'calc(100vh - 56px)', overflow: 'hidden', position: 'relative' }}>
        ...
        <>
          ...
          <aside style={{ width: 260, height: '100%', borderRight: '1px solid rgba(255,255,255,0.07)', overflowY: 'auto', flexShrink: 0, padding: '20px 0', background: 'rgba(4,4,4,0.98)', display: 'flex', flexDirection: 'column' }}>
            <style>{`
              @media(max-width:768px){.report-desktop-nav{display:none!important}}
              .report-nav-item:hover{background:rgba(255,255,255,0.04)!important}
            `}</style>

            <div className="report-desktop-nav" style={{ display: 'contents' }}>
              ...
              {FRONT_SECTIONS.map(({ id, label }) => (
                <button
                  ...
                  style={{
                    display: 'flex', alignItems: 'center', gap: 10, padding: '8px 16px', width: '100%', textAlign: 'left', background: activeSection === id ? 'rgba(255,255,255,0.06)' : 'transparent', border: 'none', transition: 'all 0.15s', cursor: 'pointer',
                  }}
                >
        ...
        {/* ── MAIN CONTENT ── */}
        <main ref={contentRef} style={{ flex: 1, overflowY: 'auto', padding: 'clamp(24px, 4vw, 48px)', background: 'var(--void)', position: 'relative' }}>

          {/* Sticky top bar */}
          <div style={{ position: 'sticky', top: 0, zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32, padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'var(--void)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <button
                onClick={() => navigate('/report')}
                style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', fontFamily: 'JetBrains Mono', fontSize: 11, cursor: 'pointer' }}
              >
                <ChevronLeft size={14} /> Report
              </button>
              <span style={{ color: 'rgba(255,255,255,0.15)', fontFamily: 'JetBrains Mono', fontSize: 11 }}>/</span>
              <span style={{ fontFamily: 'JetBrains Mono', fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>Full Document</span>
            </div>
            <button
              onClick={() => setDownloadOpen(true)}
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 18px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: 'white', fontFamily: 'Syne', fontWeight: 600, fontSize: 12, cursor: 'pointer' }}
            >
          ...
          <motion.div
            ...
            style={{ textAlign: 'center', marginBottom: 80, paddingTop: 32 }}
          >
            <p style={{ fontFamily: 'JetBrains Mono', fontSize: 11, letterSpacing: '0.2em', color: 'rgba(255,255,255,0.2)', marginBottom: 8, textTransform: 'uppercase' }}>
              Precious Cornerstone University, Ibadan
            </p>
            <p style={{ fontFamily: 'JetBrains Mono', fontSize: 11, letterSpacing: '0.2em', color: 'rgba(255,255,255,0.2)', marginBottom: 48, textTransform: 'uppercase' }}>
              Department of Cyber Security
            </p>
            <h1 style={{ fontFamily: 'Bebas Neue', fontSize: 'clamp(64px, 14vw, 120px)', letterSpacing: '0.15em', lineHeight: 0.88, marginBottom: 24, background: 'linear-gradient(135deg, #fff 0%, rgba(255,255,255,0.6) 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              DOBERMAN
            </h1>
            <p style={{ fontFamily: 'Syne', fontWeight: 500, fontSize: 'clamp(14px, 2.5vw, 20px)', color: 'rgba(255,255,255,0.5)', maxWidth: 600, margin: '0 auto 48px', lineHeight: 1.5 }}>
              Design and Implementation of a Multi-Module AI-Powered Cybersecurity Intelligence Platform
            </p>
            <div style={{ display: 'inline-block', textAlign: 'left', padding: '28px 36px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 16 }}>
              {[
                ...
                <div key={key} style={{ display: 'flex', gap: 24, marginBottom: 8, alignItems: 'baseline' }}>
                  <span style={{ fontFamily: 'JetBrains Mono', fontSize: 10, letterSpacing: '0.1em', color: 'rgba(255,255,255,0.25)', width: 80, flexShrink: 0, textTransform: 'uppercase' }}>{key}</span>
                  <span style={{ fontFamily: 'Syne', fontWeight: 600, fontSize: 14, color: 'rgba(255,255,255,0.75)' }}>{val}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <div style={{ maxWidth: 800, margin: '0 auto' }}>
            ...
            {REPORT.chapters.map((chapter) => (
              <section key={chapter.number} id={`section-ch-${chapter.number}`} style={{ marginBottom: 72 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32, paddingBottom: 16, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                  <span style={{ fontFamily: 'Bebas Neue', fontSize: 56, letterSpacing: '0.1em', color: 'rgba(255,255,255,0.06)', lineHeight: 1 }}>{chapter.number}</span>
                  <h2 style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: 'clamp(22px, 4vw, 36px)', color: 'white', lineHeight: 1.15 }}>
                    {chapter.title}
                  </h2>
                </div>
