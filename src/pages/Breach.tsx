  ...
  return (
    <div style={{ marginTop: 18 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
        {bands.map((b) => (
          <span key={b.label} style={{ fontFamily: 'Oxanium', fontSize: 8, letterSpacing: '0.08em', color: B.dim }}>{b.label}</span>
        ))}
      </div>
    </div>
  )
    ...
    <Layout profile={profile} onSignOut={signOut} title="BREACH DETECTION">
      ...
      <div style={{ position: 'relative', minHeight: 'calc(100vh - 56px)', overflow: 'hidden', background: B.void }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: 'clamp(40px,6vw,72px) clamp(20px,5vw,56px) 96px', maxWidth: 800, margin: '0 auto' }}>
          ...
          {/* Kicker */}
          <p style={{ fontFamily: 'Oxanium', fontWeight: 600, fontSize: 10, letterSpacing: '0.32em', color: B.dim, marginBottom: 20, textTransform: 'uppercase' }}>
            MODULE 04 · BREACH DETECTION
          </p>

          {/* Hero title */}
          <h1 style={{ fontFamily: 'Oxanium', fontWeight: 800, fontSize: 'clamp(48px,10vw,100px)', letterSpacing: '-0.01em', lineHeight: 0.9, color: B.heading, marginBottom: 24 }}>
            BREACH<br /><span style={{ color: B.bright }}>SCAN</span>
          </h1>

          <p style={{ fontFamily: 'Space Grotesk', fontWeight: 400, fontSize: 16, color: B.text, lineHeight: 1.7, maxWidth: 500, marginBottom: 10 }}>
            Scan any email, password, or phone number against HaveIBeenPwned and credential leak databases.
          </p>

          <p style={{ fontFamily: 'Space Grotesk', fontWeight: 500, fontSize: 11, letterSpacing: '0.1em', color: B.dim, marginBottom: 52 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 40 }}>
            <span style={{
              fontFamily: 'Oxanium', fontWeight: 700, fontSize: 11, letterSpacing: '0.12em',
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    marginBottom: -1,
                    fontFamily: 'Oxanium', fontWeight: active ? 700 : 500,
          ...
          {/* Hint */}
          <p style={{ fontFamily: 'Space Grotesk', fontWeight: 500, fontSize: 13, color: B.dim, marginBottom: 18, letterSpacing: '0.02em' }}>
            {tab.hint}
          </p>
            ...
            <div style={{ flex: 1, position: 'relative' }}>
              ...
              <input
                ...
                style={{
                  ...
                  borderRadius: 8,
                  color: B.heading, fontFamily: 'Space Grotesk', fontWeight: 500, fontSize: 15,
          ...
          <AnimatePresence>
            {error && (
              ...
              >
                <p style={{ fontFamily: 'Space Grotesk', fontSize: 13, color: B.critical }}>{error}</p>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            {result && (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 36 }}>
                  ...
                  <span style={{ fontFamily: 'Oxanium', fontWeight: 600, fontSize: 9, letterSpacing: '0.28em', color: B.dim }}>SCAN RESULT</span>
                ...
                {/* Main message */}
                <p style={{ fontFamily: 'Space Grotesk', fontWeight: 700, fontSize: 20, color: B.heading, lineHeight: 1.4, marginBottom: 32 }}>
                  {result.message}
                </p>
                ...
                {result.type === 'email' && exposureSummary && (
                  <div style={{ marginBottom: 32, padding: '18px 20px', background: mix(B.accent, 6), border: `1px solid ${B.border}`, borderRadius: 12 }}>
                    <p style={{ fontFamily: 'Oxanium', fontWeight: 700, fontSize: 9, letterSpacing: '0.26em', color: B.dim, marginBottom: 14 }}>WHAT'S EXPOSED — ACROSS ALL BREACHES</p>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: exposureSummary.sorted.length ? 14 : 0 }}>
                      {exposureSummary.sorted.map(([dc, count]) => (
                        <span key={dc} style={{ fontFamily: 'Oxanium', fontSize: 10, fontWeight: 600, color: B.bright, background: mix(B.accent, 14), border: `1px solid ${mix(B.accent, 24)}`, padding: '4px 10px', borderRadius: 4, display: 'flex', alignItems: 'center', gap: 5 }}>
                          {dc}
                    ...
                    <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap' }}>
                      <span style={{ fontFamily: 'Space Grotesk', fontSize: 12, color: B.text, display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {result.type === 'email' && (result.breach_count ?? 0) > 0 && (
                  <div style={{ marginBottom: 36 }}>
                    <p style={{ fontFamily: 'Oxanium', fontWeight: 700, fontSize: 9, letterSpacing: '0.26em', color: B.dim, marginBottom: 18 }}>
                      ...
                      <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: 14, padding: '16px 0', borderBottom: `1px solid ${mix(B.accent, 16)}` }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                            <p style={{ fontFamily: 'Space Grotesk', fontWeight: 700, fontSize: 15, color: B.heading }}>{breach.name}</p>
                            <span style={{
                              display: 'inline-flex', alignItems: 'center', gap: 3,
                              fontFamily: 'Oxanium', fontSize: 8, fontWeight: 700, letterSpacing: '0.08em',
                          ...
                          <p style={{ fontFamily: 'Space Grotesk', fontWeight: 400, fontSize: 11, color: B.dim, marginBottom: 8 }}>
                            {breach.domain}{breach.breach_date ? ` · ${breach.breach_date}` : ''}
                          </p>
                          {breach.description && (
                            <p style={{ fontFamily: 'Space Grotesk', fontWeight: 400, fontSize: 12.5, color: B.text, lineHeight: 1.6, marginBottom: breach.data_classes?.length ? 10 : 0 }}>
                          ...
                          {!!breach.data_classes?.length && (
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                              {breach.data_classes.map((dc) => (
                                <span key={dc} style={{ fontFamily: 'Oxanium', fontSize: 10, color: B.bright, background: mix(B.accent, 14), border: `1px solid ${mix(B.accent, 24)}`, padding: '2px 8px', borderRadius: 3 }}>{dc}</span>
                              ))}
                            </div>
                          )}
                        </div>
                ...
                {result.type === 'password' && result.breached && (
                  <div style={{ marginBottom: 36 }}>
                    <p style={{ fontFamily: 'Oxanium', fontWeight: 700, fontSize: 9, letterSpacing: '0.26em', color: B.dim, marginBottom: 14 }}>TIMES SEEN IN BREACH DATA</p>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 14 }}>
                      <span style={{ fontFamily: 'Oxanium', fontWeight: 800, fontSize: 54, lineHeight: 1, color: result.severity === 'critical' ? B.critical : B.warn }}>
                        {((result.occurrences as number) || 0).toLocaleString()}
                      </span>
                      <span style={{ fontFamily: 'Space Grotesk', fontSize: 13, color: B.dim }}>exposures in known databases</span>
                    </div>
                    <OccurrenceMeter occurrences={(result.occurrences as number) || 0} severity={result.severity} />
                    <p style={{ fontFamily: 'Space Grotesk', fontSize: 12.5, color: B.text, lineHeight: 1.6, marginTop: 16 }}>
                ...
                {result.type === 'phone' && (result.sources as string[] | undefined)?.length ? (
                  <div style={{ marginBottom: 36 }}>
                    <p style={{ fontFamily: 'Oxanium', fontWeight: 700, fontSize: 9, letterSpacing: '0.26em', color: B.dim, marginBottom: 16 }}>FOUND IN SOURCES</p>
                    {(result.sources as string[]).map((src, idx) => (
                      <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderBottom: `1px solid ${mix(B.accent, 16)}` }}>
                        ...
                        <span style={{ fontFamily: 'Space Grotesk', fontWeight: 400, fontSize: 14, color: B.heading }}>{src}</span>
                      </div>
                    ))}
                  </div>
                ) : null}
                ...
                {(result.recommendations as string[]).length > 0 && (
                  <div style={{ marginBottom: 40 }}>
                    <p style={{ fontFamily: 'Oxanium', fontWeight: 700, fontSize: 9, letterSpacing: '0.26em', color: B.dim, marginBottom: 16 }}>RECOMMENDED ACTIONS</p>
                    {(result.recommendations as string[]).map((rec, idx) => (
                      <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 12 }}>
                        ...
                        <p style={{ fontFamily: 'Space Grotesk', fontWeight: 400, fontSize: 14, color: B.text, lineHeight: 1.6 }}>{rec}</p>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div style={{ paddingTop: 28, borderTop: `1px solid ${mix(B.accent, 22)}` }}>
                  <p style={{ fontFamily: 'Oxanium', fontWeight: 600, fontSize: 9, letterSpacing: '0.24em', color: B.dim, marginBottom: 14 }}>WANT EXPERT ANALYSIS?</p>
                  <motion.button
                    whileHover={{ x: 4 }}
                    whileTap={{ scale: 0.97 }}
                    ...
                    style={{
                      ...
                      borderRadius: 7,
                      color: B.bright,
                      fontFamily: 'Oxanium', fontWeight: 700, fontSize: 12, letterSpacing: '0.1em',
                      cursor: 'pointer',
            ...
            {historyLoaded && history.length > 0 && (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 22 }}>
                  <div style={{ height: 1, flex: 1, background: `linear-gradient(to right, ${B.border}, transparent)` }} />
                  <span style={{ fontFamily: 'Oxanium', fontWeight: 600, fontSize: 9, letterSpacing: '0.28em', color: B.dim }}>SCAN HISTORY</span>
                  <div style={{ height: 1, flex: 1, background: `linear-gradient(to left, ${B.border}, transparent)` }} />
                </div>
                {history.map((item, i) => {
                  ...
                  return (
                    <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderBottom: i < history.length - 1 ? `1px solid ${mix(B.accent, 10)}` : 'none' }}>
                      ...
                      <p style={{ flex: 1, fontFamily: 'Space Grotesk', fontWeight: 500, fontSize: 12, color: B.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {item.scan_type.toUpperCase()}: {item.query}
                      </p>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
                        <Clock size={10} style={{ color: B.dim }} />
                        <span style={{ fontFamily: 'Oxanium', fontWeight: 500, fontSize: 10, color: B.dim }}>{fmtRel(item.created_at)}</span>
                      </div>
                    </div>
                  )
                })}
