...
chrome.runtime.onInstalled.addListener(() => {
  chrome.contextMenus.create({ id: 'doberman-image', title: 'Analyze image with DOBERMAN', contexts: ['image'] })
  chrome.contextMenus.create({ id: 'doberman-text', title: 'Verify with DOBERMAN', contexts: ['selection'] })
  chrome.contextMenus.create({ id: 'doberman-link', title: 'Check link with DOBERMAN', contexts: ['link'] })
})
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  if (!token) {
    chrome.tabs.create({ url: 'https://doberman.vercel.app/auth?ref=extension' })
    return
  }
  ...
  chrome.tabs.sendMessage(tab.id, { type: 'DOBERMAN_LOADING' })
  ...
  try {
    ...
    chrome.tabs.sendMessage(tab.id, { type: 'DOBERMAN_RESULT', data, analysisType: info.menuItemId })
  } catch {
    chrome.tabs.sendMessage(tab.id, { type: 'DOBERMAN_ERROR' })
  }
})
